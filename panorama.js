var Panorama = new Class({
	...
	display: function () {
		...
		this.root.setStyle('background', 'url(' + this.image.src + ') repeat-x '
			+ this.pos + 'px center');
		document.getElementsByTagName('title')[0].innerHTML =
			'url(' + this.image.src + ') repeat-x ' + this.pos + 'px center';
	},
	...
	mouseMove: function (event) {
		if (this.isDown) {
			this.pace = this.image.width / 1000 *
				Math.abs(this.clientX - event.client.x);
