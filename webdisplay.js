var WebDisplay=new Class({
	...
	selectPanel: function(panel,manual)
		...
		this.cars.forEach(function (tab)
			...
			if(rel&&rel.indexOf('prev')>=0)
				{
				$(tab).set('href', '#'+this.panels[(i==0?this.panels.length-1:i-1)].get('id'));
				}
			else if(rel&&rel.indexOf('next')>=0)
				{
				$(tab).set('href', '#'+this.panels[(i<this.panels.length-1?i+1:0)].get('id'));
				}
			if($(this.selectedPanel).get('id')==tab.href.substring(tab.href.indexOf('#')+1))
				{
	...
	handleCommand: function(event)
		{
		var a=event.target, found=false;
		...
		if(a!=this.root)
			...
			if($(a).get('rel')&&$(a).get('rel').indexOf('prev')>=0)
				{
				this.selectPrevious(true); found=true;
				}
			else if($(a).get('rel')&&$(a).get('rel').indexOf('next')>=0)
				{
				this.selectNext(true); found=true;
				}
			else
				{
				this.panels.forEach(function (panel)
					...
					if($(panel).get('id')==a.href.substring(a.href.indexOf('#')+1))
						{
						this.selectPanel(panel,true); found=true;
						}
					},this);
				}
			if(found)
